...
const { validateCartProducts } = require('../../../util/validateCartProducts')
const { validateCoupon, calculateCouponDiscount } = require('../../../util/validateCouponData');
const { validateAddress } = require('../../../util/validateAddress');
const AppError = require('../../../util/AppError');
const processCOD = require('../../../util/payment/codPayment');
...
module.exports = {
    ...
    confirmorder: async (req, res,next) => {
        try {
            const { paymentType, address_id, couponCode = '', clientTotal } = req.body;
            if (!paymentType) throw new AppError("Payment type is required", 400);
            const [addressResult, cartResult, couponResult] = await Promise.all([
                validateAddress(address_id),
                validateCartProducts(req.session._id),
                validateCoupon(couponCode, req.session._id) // totals can be updated after cart validation
            ]);
            const addressSnapshot = addressResult;
            const { productsForOrder, totalAmount, productDiscount } = cartResult;
            const { couponId, coupon } = couponResult;
            let validCouponDiscount= 0;
            if(couponId)
                validCouponDiscount = calculateCouponDiscount(coupon, totalAmount, productDiscount);

            const subtotal = totalAmount - productDiscount ;
            const deliverycharge = subtotal < 1000 ? 40 : 0;
            const discountAmount = productDiscount + validCouponDiscount;
            const finalAmount = subtotal - validCouponDiscount + deliverycharge;
            if (finalAmount !== clientTotal ) throw new AppError("Price mismatch. Please refresh and try again.", 409); 
            const orderId = generateOrderId();
            const currentDate = new Date();
            const expectedDelivery = new Date(currentDate.getTime() + 4 * 24 * 60 * 60 * 1000);
            ...
            const commonOrderData = {
                ...
                coupon: {
                    couponId,
                    code: couponCode,
                    discountAmount: validCouponDiscount
                },
                charges: {
            ...
            if (paymentType === "COD") {
                result = await processCOD({ finalAmount, couponId });
            }
        } catch (error) {
            next(error);
        }
